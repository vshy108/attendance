...
class CreateWorkingTemplateValidator
  Schema = Dry::Validation.Params(AppSchema) do
    ...
    # NOTE: string to integer won't remove the decimal places
    ...
    required(:valid_work_sections_attributes, Dry::Types['array'].of(Dry::Types['coercible.hash'])).each do
      schema do
        ...
        required(:from_time_in_minute, Dry::Types['coercible.integer']).filled(:int?, gt?: -720)
        # (12 + 24) x 60 = 36 x 60 = 2160, tomorrow 12pm
        required(:to_time_in_minute, Dry::Types['coercible.integer']).filled(:int?, lt?: 2160)
      end
    end

    # this validate helps check from_time_in_minute < 2160, to_time_in_minute > -720 also
